# This is a program written as solution to the Compulsory Task 2 of the Capstone Project as the tasks are slightly different
...
'''Code that will allow a user to login.
    ...
    - Use an 'if' statement to validate the user name and password
'''
...
while True:
    if username == 'admin':
        # Present the admin menu to the user and 
    ...
    : ''').lower()
        ...
        # Present the lesser functionality menu to the user and 
        # make sure that the user input is converted to lower case.
        menu = input('''Select one of the following options:
    a - add task
    va - view all tasks
    ...
    if menu == 'r':
        ...
        '''This code block will calculate the following management info of the users and tasks in each file
        1 The total number of tasks
        2 The total number of completed tasks
        3 The total number of incomplete tasks
        4 The total number of users
        5 The number of tasks each user has been assigned
        ...
        tasks_per_user = {}
        for user in user_list:
            tasks_per_user[user[0]] = 0
        for task in task_list:
            tasks_per_user[task[0]] += 1
        user_tasks = [f'{user}: {tasks_per_user[user]}' for user in tasks_per_user]
